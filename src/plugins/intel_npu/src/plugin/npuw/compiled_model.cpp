...
ov::npuw::CompiledModel::CompiledModel(const std::shared_ptr<ov::Model>& model,
    ...
    // though that tensor IS produced by some subgraph. This happens, e.g., after the
    // LLM lm-head cut (CutLMHead) renames the cut tensor: the embeddings Result and a
    // pre-existing hidden-state Result then share one source output. Point each such
    // orphan at the same {subgraph, output} as its already-linked sibling so the
    // tensor is simply broadcast to multiple model outputs (init_gio handles this).
    for (size_t j = 0; j < orig_results.size(); j++) {
